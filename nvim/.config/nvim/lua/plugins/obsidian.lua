...
--                      normalize.py --apply). Slugs the note's H1
--                      title, applies the folder-matched template
--                      body, canonicalizes frontmatter, syncs H1
--                      with aliases[0].
...
-- Return the note's body `# H1` text from a buffer, or nil when the
-- first non-blank line after the frontmatter is not an H1 (body has no
-- H1 yet). Mirrors normalize.py's extract_h1_title -- the H1 must be the
-- first non-blank body line -- so the slug derives from the same
-- "title" the normalizer syncs aliases[0] to. Used by <leader>o<space>.
local function buffer_h1(bufnr)
  local lines = vim.api.nvim_buf_get_lines(bufnr, 0, -1, false)
  local start = 1
  if lines[1] == "---" then
    start = #lines + 1 -- unclosed frontmatter: leave no body to scan
    for i = 2, #lines do
      if lines[i] == "---" then
        start = i + 1
        break
...
return {
  {
    ...
    keys = {
      ...
      -- keystroke:
      --  1. Derives the slug from the note's body H1 (the readable
      --     title; normalize.py keeps aliases[0] synced to it), falling
      --     back to the current filename stem when the body has no H1
      --     yet. Renames the file to that slug via :Obsidian rename
      --     (which rewrites every [[wikilink]] in the vault).
      ...
      -- user-added synonyms are kept; H1 reconciled with aliases[0].
      {
        ...
        function()
          ...
          local stem = vim.fn.fnamemodify(old_path, ":t:r")
          local slug = slugify(buffer_h1(0) or stem)
          ...
          if renamed then
            ...
            vim.notify("Note normalized (filename already matches H1 slug)", vim.log.levels.INFO)
          end
        end,
