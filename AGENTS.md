...
- `make verify` must fail closed across WSL2/interoperability, the command baseline, deployment ownership, resolved Git identity, and owned config syntax and runtime parsing; fixture-only overrides must not weaken normal mode.
...
- `windows-terminal/settings.json` is a full paste-ready config, never stowed; setup deploys it explicitly through idempotent, backup-first `make wt-push`.
...
- Run `make wt-diff` before and after editing or deploying `windows-terminal/settings.json`; `make wt-push` validates both files and creates an adjacent backup only when replacement is required.
