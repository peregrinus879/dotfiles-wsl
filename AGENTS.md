...
- The Makefile `TWIN_SPECS` files (nvim vault plugin specs, the `trio` workspace function) are byte-identical twins with `dotfiles-omarchy`; `make verify` fails on drift.
