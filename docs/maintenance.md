...
Coordinate assignments and stale host observations with `~/Projects/eyrie/eyragents/docs/maintenance.md` and `~/Projects/eyrie/eyrarchy/docs/maintenance.md`.
...
- after the current OpenCode session ends, remove only `~/.opencode/bin/opencode`, start a fresh shell, run `hash -r`, and verify `command -v opencode` resolves to Pacman's `/usr/bin/opencode`; launch fresh OpenCode, confirm the custom `gruvbox` theme is discovered and selectable, and keep the remaining `~/.opencode` content until separately audited.
- align EyrArcHy's `/omasync` reference-clone step and sibling-ledger pointer with this repo: fetch before `git remote set-head origin -a` so a newly selected default branch exists locally, and point its EyrWSL coordination link at `docs/maintenance.md`.
- on the Omarchy host, run EyrArcHy's `make verify` and `make lint`, then restow its changed packages; WSL static checks and twin comparisons pass, but its host-bound gates do not run from WSL.
